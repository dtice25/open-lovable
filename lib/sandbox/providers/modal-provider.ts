...
export class ModalProvider extends SandboxProvider {
  ...
  // Create compute instance with Modal provider
  ...
  // Uses appConfig.baseProviderConfig.modal.workingDirectory as the single source of truth.
  private async ensureWorkingDir(): Promise<string> {
    ...
    if (appConfig.baseProviderConfig.modal.workingDirectory) {
      this.workingDir = appConfig.baseProviderConfig.modal.workingDirectory;
    } else {
      throw new Error(
        '[ModalProvider] Unable to determine working directory. ' +
          'appConfig.baseProviderConfig.modal.workingDirectory is not set.',
      );
    }
    ...
    try {
      ...
      if (this.sandbox) {
        try {
          ...
          console.error('[ModalProvider] Failed to destroy existing sandbox:', e);
        }
        this.sandbox = null;
      ...
      // Create sandbox via ComputeSDK Modal provider
      ...
      const previewUrl = await this.sandbox.getUrl({ 
  ...
  async writeFile(path: string, content: string): Promise<void> {
    ...
    if (dirPath) {
      try {
        ...
        console.warn('[ModalProvider] Failed to ensure directory exists:', e);
      }
    }
    ...
    try {
      ...
      console.error('[ModalProvider] Failed to write file:', error);
      throw error;
    }
  }
  ...
  async setupViteApp(): Promise<void> {
    ...
    // vite.config.js - include Modal preview domain
  ...
  async terminate(): Promise<void> {
    if (this.sandbox) {
      try {
        ...
        console.error('[ModalProvider] Failed to terminate sandbox:', e);
      }
      this.sandbox = null;
